...
class AI(RealtimeAI):
	...
	def find_map(self, base):
		...
		if count == 19:
			...
			self.map = "LastStand"
			self.max_ammo_choice_cnt = {AmmoType.RifleBullet: 3, AmmoType.TankShell: 2, AmmoType.HMGBullet: 1, AmmoType.MortarShell: 5, AmmoType.GoldenTankShell: 0}
		elif count == 6:
	...
	def check_and_ban_ammo(self, base):
		for unit in base.units.values():
			...
			tank = unit.type == UnitType.Tank and (unit.health <= 2 * unit.c_individual_health or self.ammo_choice_cnt[a_type] >= self.max_ammo_choice_cnt[a_type])
			...
			mortar = unit.type == UnitType.Mortar and (unit.health <= 2 * unit.c_individual_health or self.ammo_choice_cnt[a_type] >= self.max_ammo_choice_cnt[a_type])
	...
	def choose_possible_ammo(self):
		if self.map == "AllIn":
			...
			return [AmmoType.HMGBullet] * self.max_ammo_choice_cnt[AmmoType.HMGBullet] + [AmmoType.MortarShell, AmmoType.TankShell, AmmoType.RifleBullet]
	...
	def factory_agent_handler(self, base, fagent, machines):
		if self.f_state == FactoryState.Idle:
			if self.f_pos == Position(6):
				...
				if len(self.rdy_machines) > 0:
					...
					self.factory_agent_move(self.f_forward)
						...
						machines[Position(i)].current_ammo == AmmoType.GoldenTankShell and machines[Position(i)].status == MachineStatus.Working and machines[Position(i)].construction_rem_time <= 9
						for
						i in range(7, 10)):
					self.f_state = FactoryState.PickingAmmo
					self.f_forward = True
			...
			elif len(self.ammo_to_make) == 0 and machines[self.f_pos].status == MachineStatus.Working and machines[self.f_pos].construction_rem_time <= 9:
				self.f_state = FactoryState.PickingAmmo
				return
			...
			if self.f_pos == Position(6):
				...
				self.factory_agent_put_material(self.ammo_to_make[0])
				self.ammo_to_make.pop(0)
			elif machines[self.f_pos].current_ammo == AmmoType.GoldenTankShell and machines[self.f_pos].status == MachineStatus.Working and machines[self.f_pos].construction_rem_time <= 9:
				return
			elif len(self.ammo_to_make) == 0 and machines[self.f_pos].status == MachineStatus.Working and machines[self.f_pos].construction_rem_time < 4 and all(
					machines[Position(i)].status == MachineStatus.Idle or (machines[Position(i)].status == MachineStatus.Working and machines[Position(i)].construction_rem_time > 7) for i in
					range(7, 10) if i != self.f_pos.index):
				return
			else:
				if self.should_go_for(machines[Position(9)]) or (self.should_go_for(machines[Position(8)]) and self.f_pos != Position(9)) or (
			...
			if self.f_pos == Position(6):
				...
				if self.f_pos.index in self.rdy_machines:
					self.rdy_machines.remove(self.f_pos.index)
			elif len(self.ammo_to_make) == 0 and (
					fagent.ammos_bag[AmmoType.GoldenTankShell] == 0 and machines[self.f_pos].status == MachineStatus.Working and machines[self.f_pos].construction_rem_time <= 9):
				return
			elif machines[self.f_pos].status == MachineStatus.Idle and sum(fagent.materials_bag.values()) > 0 and len(self.ammo_to_make) > 0:
				self.factory_agent_put_material(self.ammo_to_make[0])
				self.ammo_to_make.pop(0)
	...
	def should_w_wait_in_BLD(self, machines, base):
		return self.f_state == FactoryState.PickingAmmo or (self.total_f_ammo_count(base) > 0) or any(
			machines[Position(i)].current_ammo == AmmoType.GoldenTankShell and machines[Position(i)].construction_rem_time < 5 for i in range(7, 10))