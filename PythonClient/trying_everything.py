...
class AI(RealtimeAI):
	...
	def initialize(self):  # create the 9 batches of triple ammos
		...
		while len(self.batch_sequence) < 9:
			batch = list(random.choice(sorted(list(set(itertools.product([i for i in range(5)], repeat=3)))) + sorted(
				list(set(itertools.product([i for i in range(5)], repeat=2)))) + sorted(
				list(set(itertools.product([i for i in range(5)], repeat=1))))))
			if sum(self.ammo_mat_cnt[batch[i]] for i in range(len(batch))) > 15:
				continue
			hello = {}
			for ammo in batch:
